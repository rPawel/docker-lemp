...
iniSet /etc/php/7.2/mods-available/xdebug.ini "xdebug\.remote_host" $CTNR_HOST_IP
iniSet /etc/php/7.2/mods-available/xdebug.ini "xdebug\.remote_port" $CTNR_HOST_XDEBUG_PORT

if [ "$CTNR_APP_ENV" = "dev" ]; then
    echo "== CONTAINER IS STARTING IN DEV MODE =="
    phpenmod xdebug
    iniSet /etc/php/7.2/fpm/php.ini display_errors On
    iniSet /etc/php/7.2/fpm/php.ini display_startup_errors On
    iniSet /etc/php/7.2/cli/php.ini display_errors On
    iniSet /etc/php/7.2/cli/php.ini display_startup_errors On
else
    echo "== CONTAINER IS STARTING IN PROD MODE =="
    phpdismod xdebug
    iniSet /etc/php/7.2/fpm/php.ini display_errors Off
    iniSet /etc/php/7.2/fpm/php.ini display_startup_errors Off
    iniSet /etc/php/7.2/cli/php.ini display_errors Off
    iniSet /etc/php/7.2/cli/php.ini display_startup_errors Off
fi
